...
public:
	...
	void calSimi(unsigned int limit1, unsigned int limit2)
	{
		...
		{
			finishCount ++;
			for (std::size_t j = 0; j != numOfDoc; ++ j) // compare with each track
			{
...
int main()
{
	...
	cout << "Initializing multithreading..." << endl;
	int threadLen = lyric.numOfDoc / numOfThread;
	for (int i = 0; i < numOfThread - 1; ++ i)
	{
		int l1 = threadLen * i; int l2 = (i + 1) * threadLen;
		cout << "Thread " << i << " slot: " << threadLen << endl;
		workers.push_back(std::thread(&tfidf::calSimi, &lyric, l1, l2));
	}

	int l1 = (numOfThread - 1) * threadLen;
	cout << "Thread " << (numOfThread - 1) << " slot: " << (lyric.numOfDoc - l1) << endl;
	workers.push_back(std::thread(&tfidf::calSimi, &lyric, l1, lyric.numOfDoc));

	while (true)
	{
		cout << lyric.finishCount << "\r";
		std::cout.flush();
		if (lyric.finishCount == lyric.numOfDoc)
		{
			cout << endl;
